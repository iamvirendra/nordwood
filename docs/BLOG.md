...
The `/blog` route lists ten article previews. Each preview links to a dedicated `/blog/:id` page containing the complete article and its three-photo carousel. The main navigation links to the listing and stays active on article pages.
...
- The current images are illustrative placeholders, shared where relevant across articles. There are 30 gallery image placements using nine distinct local photos; the listing uses each article's first photo as its cover. They do not represent verified customer installations or the NordWood factory.
...
`src/pages/BlogDetail.jsx` and `BlogDetail.css` render an individual article with its full text, takeaway and photo carousel. A back link returns to the journal; a next-story link continues to the following article. Unrecognised article IDs show a story-not-found message and a link to the listing.
